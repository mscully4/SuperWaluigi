...
void Player::update(double delta_time, const int map_width, const int map_height, const int tile_width, const int tile_height, const int map_scale, vector<vector<int>> level) {
    ...
    const int maxHeight = ground - ((2*tile_height) * 4);
	...
	if (x_vel < 0){
	    if(level[((m_vertices[3].position.y)/(2*tile_width))][(m_vertices[3].position.x + (x_vel * delta_time))/(2*tile_width)] == 1){
            x_vel = 0;
	    }
	}
	else if (x_vel > 0){
	    if(level[((m_vertices[2].position.y)/(2*tile_width))][(m_vertices[2].position.x + (x_vel * delta_time))/(2*tile_width)] == 1){
            x_vel = 0;
	    }
	}
	...
	if (y_vel != 0){
	    if (y_vel > 0){
    	    if (level[((m_vertices[3].position.y + (y_vel * delta_time))/(2*tile_width))][(m_vertices[0].position.x + (x_vel * delta_time))/(2*tile_width)] != 1) {
    		    for (int i = 0; i < 4; ++i) {
    	    ...
    	    level[((m_vertices[0].position.y + (y_vel * delta_time))/(2*tile_width))][(m_vertices[0].position.x + (x_vel * delta_time))/(2*tile_width)] != 1) {
    		    for (int i = 0; i < 4; ++i) {
    			    m_vertices[i].position.y += y_vel * delta_time;
    		    }
    	    }
    	...
    	if (level[((m_vertices[3].position.y + delta_time)/(2*tile_width))][(m_vertices[0].position.x + (x_vel * delta_time))/(2*tile_width)] != 1) {
    	    isFalling = true;
    	    isDescending = true;
    	    y_vel = 1 * speedScale;
    	    for (int i = 0; i < 4; ++i) {
    ...
    std::cout << level[((m_vertices[3].position.y + (y_vel * delta_time))/(2*tile_width))][(m_vertices[0].position.x + (x_vel * delta_time))/(2*tile_width)] << std::endl;
